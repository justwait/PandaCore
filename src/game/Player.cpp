...
bool Player::UpdateGatherSkill(uint32 SkillId, uint32 SkillValue, uint32 RedLevel, uint32 Multiplicator )
{
    ...
    switch (SkillId)
    {
		...
		case SKILL_ARCHAEOLOGY:
            if (sWorld.getConfig(CONFIG_UINT32_SKILL_CHANCE_ARCHAEOLOGY_STEPS)==0)
                return UpdateSkillPro(SkillId, SkillGainChance(SkillValue, RedLevel+100, RedLevel+50, RedLevel+25)*Multiplicator,gathering_skill_gain);
            else
                return UpdateSkillPro(SkillId, (SkillGainChance(SkillValue, RedLevel+100, RedLevel+50, RedLevel+25)*Multiplicator) >> (SkillValue/sWorld.getConfig(CONFIG_UINT32_SKILL_CHANCE_ARCHAEOLOGY_STEPS)),gathering_skill_gain);
    }
    return false;
}
...
void Player::SendInitWorldStates(uint32 zoneid, uint32 areaid)
{
    ...
    switch(zoneid)
    {
        ...
        case 6126:                                          // SM
            ...
            break;
        case 6215:                                          // VP
            if (bg && bg->GetTypeID() == BATTLEGROUND_TK)
                bg->FillInitialWorldStates(data, count);
            else
