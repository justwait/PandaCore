...
uint32 GetSpellCastTime(SpellEntry const* spellInfo, Spell const* spell)
{
    ...
    if (spell)
    {
        ...
        if (!(spellMisc->Attributes & (SPELL_ATTR_UNK4|SPELL_ATTR_TRADESPELL)))
    ...
    if (spellMisc->Attributes & SPELL_ATTR_RANGED && (!spell || !spell->IsAutoRepeat()))
        castTime += 500;
...
WeaponAttackType GetWeaponAttackType(SpellEntry const *spellInfo)
{
    ...
    switch (spellInfo->GetDmgClass())
    {
        case SPELL_DAMAGE_CLASS_MELEE:
            if (spellMisc && (spellMisc->AttributesEx3 & SPELL_ATTR_EX3_REQ_OFFHAND))
                return OFF_ATTACK;
            else
                return BASE_ATTACK;
            break;
        ...
        default:
                                                            // Wands
            if (spellMisc && (spellMisc->AttributesEx2 & SPELL_ATTR_EX2_AUTOREPEAT_FLAG))
                return RANGED_ATTACK;
            else
                return BASE_ATTACK;
            break;
...
{
    return (sSpellMiscStore.LookupEntry(spellInfo->Id)->Attributes & SPELL_ATTR_PASSIVE) != 0;
}
...
SpellSpecific GetSpellSpecific(uint32 spellId)
{
    ...
    switch(spellInfo->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
        {
            ...
            else
            {
                ...
                if ((spellMisc && (spellMisc->AttributesEx2 & SPELL_ATTR_EX2_FOOD_BUFF) || spellMisc->SpellIconID == 2560))
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            if ((spellMisc && spellMisc->Attributes & SPELL_ATTR_CASTABLE_WHILE_SITTING) &&
                (interrupts && interrupts->InterruptFlags & SPELL_INTERRUPT_FLAG_AUTOATTACK) &&
                (spellMisc && spellMisc->SpellIconID == 52 || spellMisc && spellMisc->SpellIconID == 79))
                return SPELL_WELL_FED;
            break;
        }
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            if (spellInfo->IsFitToFamilyMask(UI64LIT(0x00000820180400)) && (spellMisc && spellMisc->AttributesEx3 & 0x200) && (spellMisc && spellMisc->SpellIconID != 237))
    ...
    if ((IsSpellHaveAura(spellInfo, SPELL_AURA_TRACK_CREATURES) ||
        ...
        ((spellMisc && spellMisc->AttributesEx & SPELL_ATTR_EX_UNK17) || (spellMisc && spellMisc->AttributesEx6 & SPELL_ATTR_EX6_UNK12)))
...
bool IsPositiveEffect(SpellEntry const *spellproto, SpellEffectIndex effIndex)
{
        ...
        case SPELL_EFFECT_APPLY_AREA_AURA_FRIEND:
        {
            switch(spellEffect->EffectApplyAuraName)
            {
                ...
                case SPELL_AURA_MOD_DECREASE_SPEED:         // used in positive spells also
                    ...
                    if (spellMisc && spellMisc->Attributes & 0x4000000 && effIndex == EFFECT_INDEX_0)
                        return false;
                    break;
    ...
    // AttributesEx check
    if(spellMisc && spellMisc->AttributesEx & SPELL_ATTR_EX_NEGATIVE)
        return false;
...
bool IsSingleTargetSpell(SpellEntry const *spellInfo)
{
    ...
    if ( spellMisc->AttributesEx5 & SPELL_ATTR_EX5_SINGLE_TARGET_SPELL )
        return true;
...
{
    SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(spellInfo1->Id);
    SpellMiscEntry const* spellMisc2 = sSpellMiscStore.LookupEntry(spellInfo2->Id);
    // TODO - need better check
    // Equal icon and spellfamily
    if( spellInfo1->GetSpellFamilyName() == spellInfo2->GetSpellFamilyName() &&
        spellMisc->SpellIconID == spellMisc2->SpellIconID )
        return true;
...
SpellCastResult GetErrorAtShapeshiftedCast (SpellEntry const *spellInfo, uint32 form)
{
    ...
    if(actAsShifted)
    {
        if (spellMisc && spellMisc->Attributes & SPELL_ATTR_NOT_SHAPESHIFT) // not while shapeshifted
            return SPELL_FAILED_NOT_SHAPESHIFT;
    ...
    {
        // needs shapeshift
        if(!(spellMisc && spellMisc->AttributesEx2 & SPELL_ATTR_EX2_NOT_NEED_SHAPESHIFT) && shapeShift && shapeShift->Stances != 0)
            return SPELL_FAILED_ONLY_SHAPESHIFT;
    }
...
bool SpellMgr::IsSpellProcEventCanTriggeredBy(SpellProcEventEntry const * spellProcEvent, uint32 EventProcFlag, SpellEntry const * procSpell, uint32 procFlags, uint32 procExtra)
{
    ...
    if (spellProcEvent)     // Exist event data
    {
        ...
        else // For spells need check school/spell family/family mask
        {
            ...
            if(spellProcEvent->schoolMask && (spellProcEvent->schoolMask & spellMisc->SchoolMask) == 0)
                return false;
            ...
            case SPELLFAMILY_PALADIN:
                {
                    ...
                    if (classOptions && (classOptions->SpellFamilyFlags & UI64LIT(0x0000000008000000)) && spellMisc->SpellIconID == 25)
                        return false;
                }
                break;
...
bool SpellMgr::IsNoStackSpellDueToSpell(uint32 spellId_1, uint32 spellId_2) const
{
    ...
    SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(spellInfo_1->Id);
    SpellMiscEntry const* spellMisc2 = sSpellMiscStore.LookupEntry(spellInfo_2->Id);
    ...
    if ((spellMisc->Attributes & SPELL_ATTR_PASSIVE) != (spellMisc2->Attributes & SPELL_ATTR_PASSIVE))
        return false;
    ...
    switch(spellInfo_1->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
            switch(spellInfo_2->GetSpellFamilyName())
            {
                case SPELLFAMILY_GENERIC:                   // same family case
                {
                    ...
                    if (spellMisc->SpellIconID == 92 && spellMisc2->SpellIconID == 92 && (
                        (spellMisc->SpellVisual[0] == 99 && spellMisc2->SpellVisual[0] == 0) ||
                        (spellMisc2->SpellVisual[0] == 99 && spellMisc->SpellVisual[0] == 0)))
                        return false;

                    // Heart of the Wild, Agility and various Idol Triggers
                    if (spellMisc->SpellIconID == 240 && spellMisc2->SpellIconID == 240)
                        return false;

                    // Personalized Weather (thunder effect should overwrite rainy aura)
                    if (spellMisc->SpellIconID == 2606 && spellMisc2->SpellIconID == 2606)
                        return false;

                    // Mirrored Soul (FoS - Devourer) - and other Boss spells
                    if (spellMisc->SpellIconID == 3176 && spellMisc2->SpellIconID == 3176)
                        return false;
                    ...
                    if (spellMisc->SpellIconID == 1662 && spellMisc2->SpellIconID == 1662)
                        return false;

                    // Kindred Spirits
                    if (spellMisc->SpellIconID == 3559 && spellMisc2->SpellIconID == 3559)
                        return false;

                    // Vigilance and Damage Reduction (Vigilance triggered spell)
                    if (spellMisc->SpellIconID == 2834 && spellMisc2->SpellIconID == 2834)
                        return false;
                ...
                case SPELLFAMILY_MAGE:
                    ...
                    if (spellMisc2->SpellIconID == 125 && spellInfo_1->Id == 18820)
                        return false;
                    break;
                case SPELLFAMILY_WARRIOR:
                {
                    // Scroll of Protection and Defensive Stance (multi-family check)
                    if (spellMisc->SpellIconID == 276 && spellMisc->SpellVisual[0] == 196 && spellInfo_2->Id == 71)
                        return false;
				...
				case SPELLFAMILY_DRUID:
                {
                    ...
                    if (spellMisc->SpellIconID == 312 && spellMisc->SpellVisual[0] == 216 && spellInfo_2->Id == 24932)
                        return false;
                ...
                case SPELLFAMILY_ROGUE:
                {
                    ...
                    if (spellMisc->SpellIconID == 498 && spellMisc->SpellVisual[0] == 0 && spellMisc2->SpellIconID == 498)
                        return false;
                ...
                case SPELLFAMILY_PALADIN:
                {
                    ...
                    if (spellMisc2->SpellIconID==502 && spellMisc->SpellIconID==502 && spellMisc->SpellVisual[0]==969)
                        return false;

                    // *Band of Eternal Champion and Seal of Command(multi-family check)
                    if (spellId_1 == 35081 && spellMisc2->SpellIconID==561 && spellMisc2->SpellVisual[0]==7992)
                        return false;
            ...
            if(spellMisc->SpellIconID == 1691 && spellMisc2->SpellIconID == 1691)
                return false;
            break;
        case SPELLFAMILY_MAGE:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_MAGE )
            {
                ...
                if (classOptions1 && (classOptions1->SpellFamilyFlags & UI64LIT(0x0000000000010000)) && (spellMisc2->SpellVisual[0] == 72 && spellMisc2->SpellIconID == 1499) ||
                    (classOptions2->SpellFamilyFlags & UI64LIT(0x0000000000010000)) && (spellMisc->SpellVisual[0] == 72 && spellMisc->SpellIconID == 1499) )
                    return false;

                // Fingers of Frost effects
                if (spellMisc->SpellIconID == 2947 && spellMisc2->SpellIconID == 2947)
                    return false;
            ...
            if (spellInfo_2->Id == 132 && spellMisc->SpellIconID == 209 && spellMisc->SpellVisual[0] == 968)
                return false;

            // Combustion and Fire Protection Aura (multi-family check)
            if (spellInfo_1->Id == 11129 && spellMisc2->SpellIconID == 33 && spellMisc2->SpellVisual[0] == 321)
                return false;

            // Arcane Intellect and Insight
            if (spellMisc->SpellIconID == 125 && spellInfo_2->Id == 18820)
                return false;
        ...
        case SPELLFAMILY_WARLOCK:
            if( classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_WARLOCK )
            {
                ...
                if ((spellMisc->SpellIconID == 152 && spellMisc2->SpellIconID == 546) ||
                    (spellMisc2->SpellIconID == 152 && spellMisc->SpellIconID == 546))
                    return false;

                //Corruption & Seed of corruption
                if ((spellMisc->SpellIconID == 313 && spellMisc2->SpellIconID == 1932) ||
                    (spellMisc2->SpellIconID == 313 && spellMisc->SpellIconID == 1932))
                    if(spellMisc->SpellVisual[0] != 0 && spellMisc2->SpellVisual[0] != 0)
                        return true;                        // can't be stacked

                // Corruption and Unstable Affliction
                if ((spellMisc->SpellIconID == 313 && spellMisc2->SpellIconID == 2039) ||
                    (spellMisc2->SpellIconID == 313 && spellMisc->SpellIconID == 2039))
                    return false;

                // (Corruption or Unstable Affliction) and (Curse of Agony or Curse of Doom)
                if (((spellMisc->SpellIconID == 313 || spellMisc->SpellIconID == 2039) && (spellMisc2->SpellIconID == 544  || spellMisc2->SpellIconID == 91)) ||
                    ((spellMisc2->SpellIconID == 313 || spellMisc2->SpellIconID == 2039) && (spellMisc->SpellIconID == 544  || spellMisc->SpellIconID == 91)))
                    return false;

                // Shadowflame and Curse of Agony
                if( spellMisc->SpellIconID == 544 && spellMisc2->SpellIconID == 3317 ||
                    spellMisc2->SpellIconID == 544 && spellMisc->SpellIconID == 3317 )
                    return false;

                // Shadowflame and Curse of Doom
                if( spellMisc->SpellIconID == 91 && spellMisc2->SpellIconID == 3317 ||
                    spellMisc2->SpellIconID == 91 && spellMisc->SpellIconID == 3317 )
                    return false;

                // Metamorphosis, diff effects
                if (spellMisc->SpellIconID == 3314 && spellMisc2->SpellIconID == 3314)
                    return false;
            }
            // Detect Invisibility and Mana Shield (multi-family check)
            if (spellInfo_1->Id == 132 && spellMisc2->SpellIconID == 209 && spellMisc2->SpellVisual[0] == 968)
                return false;
            break;
        case SPELLFAMILY_WARRIOR:
            if (classOptions2 && classOptions1->SpellFamilyName == SPELLFAMILY_WARRIOR )
            {
                ...
                if ((spellMisc->SpellIconID == 456 && spellMisc2->SpellIconID == 2006) ||
                    (spellMisc2->SpellIconID == 456 && spellMisc->SpellIconID == 2006))
                    return false;

                // Glyph of Revenge (triggered), and Sword and Board (triggered)
                if ((spellMisc->SpellIconID == 856 && spellMisc2->SpellIconID == 2780) ||
                    (spellMisc2->SpellIconID == 856 && spellMisc->SpellIconID == 2780))
                    return false;
            ...
            if (spellInfo_1->Id == 71 && spellMisc2->SpellIconID == 276 && spellMisc2->SpellVisual[0] == 196)
                return false;

            // Bloodlust and Bloodthirst (multi-family check)
            if (spellInfo_2->Id == 2825 && spellMisc->SpellIconID == 38 && spellMisc->SpellVisual[0] == 0)
                return false;
        ...
        case SPELLFAMILY_DRUID:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_DRUID )
            {
                ...
                if ((classOptions1 && (classOptions1->SpellFamilyFlags == UI64LIT(0x0) && spellMisc->SpellIconID == 108) && (classOptions2->SpellFamilyFlags & UI64LIT(0x20000000000000))) ||
                    ((classOptions2->SpellFamilyFlags == UI64LIT(0x0) && spellMisc2->SpellIconID == 108) && (classOptions1->SpellFamilyFlags & UI64LIT(0x20000000000000))))
                    return false;
                ...
                if ((spellMisc->SpellIconID == 2101 && spellMisc2->SpellIconID == 2864) ||
                    (spellMisc2->SpellIconID == 2101 && spellMisc->SpellIconID == 2864))
                    return false;

                //  Innervate and Glyph of Innervate and some other spells
                if (spellMisc->SpellIconID == 62 && spellMisc2->SpellIconID == 62)
                    return false;
                ...
                if (spellMisc->SpellIconID == 2865 && spellMisc2->SpellIconID == 2865)
                    return false;
            ...
            if (spellInfo_1->Id == 24932 && spellMisc2->SpellIconID == 312 && spellMisc2->SpellVisual[0] == 216)
                return false;
        ...
        case SPELLFAMILY_ROGUE:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_ROGUE )
            {
                ...
                if (spellMisc->SpellIconID == 516 && spellMisc2->SpellIconID == 516 &&
            ...
            //Overkill
            if (spellMisc->SpellIconID == 2285 && spellMisc2->SpellIconID == 2285)
                return false;

            // Garrote -> Garrote-Silence (multi-family check)
            if (spellMisc->SpellIconID == 498 && spellMisc2->SpellIconID == 498 && spellMisc2->SpellVisual[0] == 0)
                return false;
            break;
        case SPELLFAMILY_HUNTER:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_HUNTER )
            {
                ...
                // Deterrence
                if (spellMisc->SpellIconID == 83 && spellMisc2->SpellIconID == 83)
                    return false;

                // Bestial Wrath
                if (spellMisc->SpellIconID == 1680 && spellMisc2->SpellIconID == 1680)
                    return false;

                // Aspect of the Viper & Vicious Viper
                if (spellMisc->SpellIconID == 2227 && spellMisc2->SpellIconID == 2227)
                    return false;
            }
        ...
        case SPELLFAMILY_PALADIN:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_PALADIN )
            {
                ...
                if ((spellInfo_1->IsFitToFamilyMask(UI64LIT(0x0), 0x00000020) && (spellMisc2->SpellIconID == 291 || spellMisc2->SpellIconID == 3028)) ||
                    (spellInfo_2->IsFitToFamilyMask(UI64LIT(0x0), 0x00000020) && (spellMisc->SpellIconID == 291 || spellMisc->SpellIconID == 3028)))
                    return false;

                // Beacon of Light and Light's Beacon
                if ((spellMisc->SpellIconID == 3032) && (spellMisc2->SpellIconID == 3032))
                    return false;

                // Concentration Aura and Improved Concentration Aura and Aura Mastery
                if ((spellMisc->SpellIconID == 1487) && (spellMisc2->SpellIconID == 1487))
                    return false;

                // Seal of Corruption (caster/target parts stacking allow, other stacking checked by spell specs)
                if (spellMisc->SpellIconID == 2292 && spellMisc2->SpellIconID == 2292)
                    return false;

                // Divine Sacrifice and Divine Guardian
                if (spellMisc->SpellIconID == 3837 && spellMisc2->SpellIconID == 3837)
                    return false;

                // Blood Corruption, Holy Vengeance, Righteous Vengeance
                if ((spellMisc->SpellIconID == 2292 && spellMisc2->SpellIconID == 3025) ||
                    (spellMisc2->SpellIconID == 2292 && spellMisc->SpellIconID == 3025))
                    return false;
            }
            ...
            // Combustion and Fire Protection Aura (multi-family check)
            if (spellInfo_2->Id == 11129 && spellMisc->SpellIconID == 33 && spellMisc->SpellVisual[0] == 321)
                return false;

            // *Sanctity Aura -> Unstable Currents and other (multi-family check)
            if (spellMisc->SpellIconID==502 && classOptions2->SpellFamilyName == SPELLFAMILY_GENERIC && spellMisc2->SpellIconID==502 && spellMisc2->SpellVisual[0]==969 )
                return false;

            // *Seal of Command and Band of Eternal Champion (multi-family check)
            if (spellMisc->SpellIconID==561 && spellMisc->SpellVisual[0]==7992 && spellId_2 == 35081)
                return false;
            break;
        case SPELLFAMILY_SHAMAN:
            ...
            {
                // Windfury weapon
                if (spellMisc->SpellIconID==220 && spellMisc2->SpellIconID==220 &&
                    !classOptions1->IsFitToFamilyMask(classOptions2->SpellFamilyFlags))
                    return false;

                // Ghost Wolf
                if (spellMisc->SpellIconID == 67 && spellMisc2->SpellIconID == 67)
                    return false;

                // Totem of Wrath (positive/negative), ranks checked early
                if (spellMisc->SpellIconID == 2019 && spellMisc2->SpellIconID == 2019)
                    return false;
            }
            // Bloodlust and Bloodthirst (multi-family check)
            if (spellInfo_1->Id == 2825 && spellMisc2->SpellIconID == 38 && spellMisc2->SpellVisual[0] == 0)
                return false;
            break;
        case SPELLFAMILY_DEATHKNIGHT:
            if (classOptions2 && classOptions2->SpellFamilyName == SPELLFAMILY_DEATHKNIGHT)
            {
                ...
                if (spellMisc->SpellIconID == 61 && spellMisc2->SpellIconID == 61)
                    return false;

                // Frost Presence and Frost Presence (triggered)
                if (spellMisc->SpellIconID == 2632 && spellMisc2->SpellIconID == 2632)
                    return false;

                // Unholy Presence and Unholy Presence (triggered)
                if (spellMisc->SpellIconID == 2633 && spellMisc2->SpellIconID == 2633)
                    return false;

                // Blood Presence and Blood Presence (triggered)
                if (spellMisc->SpellIconID == 2636 && spellMisc2->SpellIconID == 2636)
                    return false;
            }
            break;
        default:
    ...
    // more generic checks
    if (spellMisc->SpellIconID == spellMisc2->SpellIconID &&
        spellMisc->SpellIconID != 0 && spellMisc2->SpellIconID != 0)
    {
...
SpellCastResult SpellMgr::GetSpellAllowedInLocationError(SpellEntry const *spellInfo, uint32 map_id, uint32 zone_id, uint32 area_id, Player const* player)
{
    ...
    if ((spellMisc && spellMisc->AttributesEx4 & SPELL_ATTR_EX4_CAST_ONLY_IN_OUTLAND) && !(player && player->isGameMaster()))
    {
    ...
    if (spellMisc && spellMisc->AttributesEx6 & SPELL_ATTR_EX6_NOT_IN_RAID_INSTANCE)
    {
    ...
    if ((spellMisc && spellMisc->AttributesEx4 & SPELL_ATTR_EX4_NOT_USABLE_IN_ARENA) ||
         (GetSpellRecoveryTime(spellInfo) > 10 * MINUTE * IN_MILLISECONDS && !(spellMisc && spellMisc->AttributesEx4 & SPELL_ATTR_EX4_USABLE_IN_ARENA)))
        if (player && player->InArena())
            return SPELL_FAILED_NOT_IN_ARENA;

    // Spell casted only on battleground
    if ((spellMisc && spellMisc->AttributesEx3 & SPELL_ATTR_EX3_BATTLEGROUND))
...
void SpellMgr::CheckUsedSpells(char const* table)
{
    ...
    do
    {
        ...
        if(spell)
        {
            ...
            SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(spellEntry->Id);
            ...
            if (spellIcon >= 0 && spellMisc->SpellIconID != uint32(spellIcon))
            {
                sLog.outError("Spell %u '%s' icon(%u) <> %u but used in %s.",spell,name.c_str(),spellMisc->SpellIconID,spellIcon,code.c_str());
                continue;
            }

            if (spellVisual >= 0 && spellMisc->SpellVisual[0] != uint32(spellVisual))
            {
                sLog.outError("Spell %u '%s' visual(%u) <> %u but used in %s.",spell,name.c_str(),spellMisc->SpellVisual[0],spellVisual,code.c_str());
                continue;
            }
...
DiminishingGroup GetDiminishingReturnsGroupForSpell(SpellEntry const* spellproto, bool triggered)
{
    ...
    switch(spellproto->GetSpellFamilyName())
    {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            else if (spellMisc->SpellIconID == 163)
        ...
        case SPELLFAMILY_HUNTER:
        {
            ...
            if  (spellMisc->SpellIconID == 180 || spellMisc->SpellIconID == 1721)
                return DIMINISHING_DISORIENT;
            break;
        }
        case SPELLFAMILY_WARLOCK:
