...
SpellAuraProcResult Unit::HandleDummyAuraProc(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const * procSpell, uint32 procFlag, uint32 procEx, uint32 cooldown)
{
    ...
    switch(dummySpell->GetSpellFamilyName())
    {
        ...
        {
            // Magic Absorption
            if (spellMisc->SpellIconID == 459)             // only this spell have SpellIconID == 459 and dummy aura
            {
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (dummyClassOptions && dummyClassOptions->SpellFamilyFlags == UI64LIT(0x0) && spellMisc->SpellIconID == 1932)
            {
                Modifier* mod = triggeredByAura->GetModifier();
        ...
        case SPELLFAMILY_DRUID:
        {
            switch(dummySpell->Id)
            {
                ...
                case 70664:
                {
                    ...
                    if (spellEffect && spellEffect->EffectRadiusIndex)
                        ...
                        radius = GetSpellMaxRange(sSpellRangeStore.LookupEntry(pSpellMisc->rangeIndex));
            ...
            // Living Seed
            else if (spellMisc->SpellIconID == 2860)
            {
        ...
        case SPELLFAMILY_ROGUE:
        {
            ...
            // Deadly Brew
            if (spellMisc->SpellIconID == 2963)
            {
                triggered_spell_id = 44289;
                break;
            }
            // Quick Recovery
            if (spellMisc->SpellIconID == 2116)
            {
                if(!procSpell)
                    return SPELL_AURA_PROC_FAILED;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Lightning Overload
            if (spellMisc->SpellIconID == 2018)            // only this spell have SpellFamily Shaman SpellIconID == 2018 and dummy aura
            {
...
SpellAuraProcResult Unit::HandleProcTriggerSpellAuraProc(Unit *pVictim, uint32 damage, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 procFlags, uint32 procEx, uint32 cooldown)
{
    ...
    switch (auraSpellInfo->GetSpellFamilyName())
    {
        case SPELLFAMILY_GENERIC:
            ...
            break;
        case SPELLFAMILY_MAGE:
            if (spellMisc->SpellIconID == 2127)         // Blazing Speed
            {
                switch (auraSpellInfo->Id)
                {
                    case 31641:  // Rank 1
        ...
        case SPELLFAMILY_WARRIOR:
            ...
            if (auraClassOptions && auraClassOptions->SpellFamilyFlags == UI64LIT(0x0) && spellMisc->SpellIconID == 243)
            {
                float weaponDamage;
            ...
            else if (spellMisc->SpellIconID == 2961)    // Taste for Blood
            {
                // only at real damage
                if (!damage)
                    return SPELL_AURA_PROC_FAILED;
        ...
        case SPELLFAMILY_WARLOCK:
        {
            ...
            if (auraSpellInfo->IsFitToFamilyMask(UI64LIT(0x0000000000004000)))
            {
                ...
                for(Unit::AuraList::const_iterator i = mDummyAura.begin(); i != mDummyAura.end(); ++i)
                {
                    ...
                    if ((*i)->GetSpellProto()->GetSpellFamilyName() == SPELLFAMILY_WARLOCK && spellMisc->SpellIconID == 113)
                    {
            ...
            // Nether Protection
            else if (spellMisc->SpellIconID == 1985)
            {
                if (!procSpell)
                    return SPELL_AURA_PROC_FAILED;
                switch(GetFirstSchoolInMask(GetSpellSchoolMask(procSpell)))
        ...
        case SPELLFAMILY_PRIEST:
        {
            ...
            // Blessed Recovery
            else if (spellMisc->SpellIconID == 1875)
            {
                switch (auraSpellInfo->Id)
                {
        ...
        case SPELLFAMILY_ROGUE:
        {
            if (spellMisc->SpellIconID == 2260)         // Combat Potency
            {
        ...
        {
            // Piercing Shots
            if (spellMisc->SpellIconID == 3247 && spellMisc->SpellVisual[0] == 0)
            {
            ...
            // Lock and Load
            else if (spellMisc->SpellIconID == 3579)
            {
        ...
        case SPELLFAMILY_PALADIN:
        {
            ...
            */
            ...
            // Illumination
            else if (spellMisc->SpellIconID==241)
            {
                if(!procSpell)
                    return SPELL_AURA_PROC_FAILED;
        ...
        case SPELLFAMILY_SHAMAN:
        {
            ...
            // Nature's Guardian
            else if (spellMisc->SpellIconID == 2013)
            {
        ...
        case SPELLFAMILY_DEATHKNIGHT:
        {
            ...
            // Blade Barrier
            else if (spellMisc->SpellIconID == 85)
            {
...
SpellAuraProcResult Unit::HandleOverrideClassScriptAuraProc(Unit *pVictim, uint32 /*damage*/, Aura *triggeredByAura, SpellEntry const *procSpell, uint32 /*procFlag*/, uint32 /*procEx*/ ,uint32 cooldown)
{
    ...
    switch(scriptId)
    {
        ...
        {
            if (!procSpell || spellMisc->SpellVisual[0]!=9487)
                return SPELL_AURA_PROC_FAILED;
...
SpellAuraProcResult Unit::HandleModPowerCostSchoolAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const* procSpell, uint32 /*procFlag*/, uint32 /*procEx*/, uint32 /*cooldown*/)
{
    ...
    return !(procSpell == NULL ||
            ...
            (triggeredByAura->GetModifier()->m_miscvalue & spellMisc->SchoolMask) == 0) ? SPELL_AURA_PROC_OK : SPELL_AURA_PROC_FAILED;  // School check
}
...
SpellAuraProcResult Unit::HandleModDamagePercentDoneAuraProc(Unit* /*pVictim*/, uint32 /*damage*/, Aura* triggeredByAura, SpellEntry const *procSpell, uint32 /*procFlag*/, uint32 procEx, uint32 cooldown)
{
    ...
    // Arcane Blast
    else if (spellInfo->Id == 36032 && procSpell->GetSpellFamilyName() == SPELLFAMILY_MAGE && spellMisc->SpellIconID == 2294)
