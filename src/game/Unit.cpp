    ...
    if (!(damageInfo->HitInfo & HITINFO_MISS))
    {
        ...
        for(AuraList::const_iterator i = vDamageShields.begin(); i != vDamageShields.end();)
        {
            if (alreadyDone.find(*i) == alreadyDone.end())
            {
                ...
                SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(i_spellProto->Id);
                if (!spellMisc)
                    continue;
                ...
                data << uint32(spellMisc->SchoolMask);
...
void Unit::CalculateDamageAbsorbAndResist(Unit *pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, const uint32 damage, uint32 *absorb, uint32 *resist, bool canReflect)
{
    ...
    for(AuraList::const_iterator i = vAbsorb.begin(); i != vAbsorb.end() && RemainingDamage > 0; ++i)
    {
        ...
        if(adsClassOptions && adsClassOptions->SpellFamilyName == SPELLFAMILY_MAGE && adsClassOptions->SpellFamilyFlags & UI64LIT(0x0000000000000108))
        {
            ...
            for (Unit::AuraList::const_iterator itr = auras.begin(); itr != auras.end(); ++itr)
            {
                ...
                SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(itr_spellProto->Id);
                if (!spellMisc)
                    continue;

                // Frost Warding (chance full absorb)
                if (itr_spellProto->GetSpellFamilyName() == SPELLFAMILY_MAGE && spellMisc->SpellIconID == 501)
                {
    ...
    SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(preventDeathSpell->Id);
...
void Unit::CalculateAbsorbResistBlock(Unit *pCaster, SpellNonMeleeDamage *damageInfo, SpellEntry const* spellProto, WeaponAttackType attType)
{
    ...
    CalculateDamageAbsorbAndResist(pCaster, GetSpellSchoolMask(spellProto), SPELL_DIRECT_DAMAGE, absorb_affected_damage, &damageInfo->absorb, &damageInfo->resist, !(spellMisc->AttributesEx2 & SPELL_ATTR_EX2_CANT_REFLECTED));
    damageInfo->damage-= damageInfo->absorb + damageInfo->resist;
}
...
bool Unit::IsSpellBlocked(Unit *pCaster, SpellEntry const *spellEntry, WeaponAttackType attackType)
{
    ...
    if (spellEntry)
    {
        ...
        if (spellMisc->Attributes & SPELL_ATTR_IMPOSSIBLE_DODGE_PARRY_BLOCK)
            return false;
    }
...
SpellMissInfo Unit::MeleeSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    // Roll miss
    uint32 tmp = spellMisc->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS ? 0 : missChance;
    if (roll < tmp)
...
SpellMissInfo Unit::MagicSpellHitResult(Unit *pVictim, SpellEntry const *spell)
{
    ...
    int32 tmp = spellMisc->AttributesEx3 & SPELL_ATTR_EX3_CANT_MISS ? 0 : (10000 - HitChance);
...
bool Unit::AddSpellAuraHolder(SpellAuraHolder *holder)
{
    ...
    if ((!holder->IsPassive() && !holder->IsPersistent()) || holder->IsAreaAura())
    {
        ...
        for (SpellAuraHolderMap::iterator iter = spair.first; iter != spair.second; ++iter)
        {
            ...
            SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(foundHolder->GetSpellProto()->Id);
            if (foundHolder->GetCasterGuid() == holder->GetCasterGuid())
            {
                ...
                if (spellMisc->AttributesEx4 & SPELL_ATTR_EX4_STACK_DOT_MODIFIER)
                {
...
void Unit::RemoveAurasWithAttribute(uint32 flags)
{
    for (SpellAuraHolderMap::iterator iter = m_spellAuraHolders.begin(); iter != m_spellAuraHolders.end(); )
    {
        SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(iter->second->GetSpellProto()->Id);
        if (!spellMisc)
            continue;

        if (spellMisc->Attributes & flags)
        {
            RemoveSpellAuraHolder(iter->second);
            iter = m_spellAuraHolders.begin();
        }
...
void Unit::RemoveArenaAuras(bool onleave)
{
    ...
    for(SpellAuraHolderMap::iterator iter = m_spellAuraHolders.begin(); iter != m_spellAuraHolders.end();)
    {
        ...
        if (!(spellMisc->AttributesEx4 & SPELL_ATTR_EX4_UNK21) &&
                                                            // don't remove stances, shadowform, pally/hunter auras
            !iter->second->IsPassive() &&                   // don't remove passive auras
            (!(spellMisc->Attributes & SPELL_ATTR_UNAFFECTED_BY_INVULNERABILITY) ||
            !(spellMisc->Attributes & SPELL_ATTR_UNK8)) &&
...
void Unit::AddGameObject(GameObject* gameObj)
{
    ...
    if ( GetTypeId()==TYPEID_PLAYER && gameObj->GetSpellId() )
    {
        ...
        SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(createBySpell->Id);
        // Need disable spell use for owner
        if (createBySpell && spellMisc->Attributes & SPELL_ATTR_DISABLED_WHILE_ACTIVE)
...
uint32 Unit::SpellDamageBonusDone(Unit *pVictim, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    if(!spellProto || !pVictim || damagetype==DIRECT_DAMAGE || spellMisc->AttributesEx6 & SPELL_ATTR_EX6_NO_DMG_MODS)
        return pdamage;
    ...
    for(AuraList::const_iterator i = mOverrideClassScript.begin(); i != mOverrideClassScript.end(); ++i)
    {
        ...
        if (!(*i)->isAffectedOnSpell(spellProto) || !spellMisc)
            continue;
...
uint32 Unit::SpellDamageBonusTaken(Unit *pCaster, SpellEntry const *spellProto, uint32 pdamage, DamageEffectType damagetype, uint32 stack)
{
    ...
    SpellMiscEntry const* spellMisc = sSpellMiscStore.LookupEntry(spellProto->Id);
    uint32 schoolMask = spellMisc->SchoolMask;
...
bool Unit::IsSpellCrit(Unit *pVictim, SpellEntry const *spellProto, SpellSchoolMask schoolMask, WeaponAttackType attackType)
{
    ...
    // not critting spell
    if((spellMisc->AttributesEx2 & SPELL_ATTR_EX2_CANT_CRIT))
        return false;
        ...
        case SPELL_DAMAGE_CLASS_MAGIC:
        {
            ...
            if (pVictim)
            {
                ...
                switch(spellProto->GetSpellFamilyName())
                {
                    ...
                    {
                        // Fire Blast
                        if (spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000002)) && spellMisc->SpellIconID == 12)
                        {
                            // Glyph of Fire Blast
...
bool Unit::IsImmuneToSpell(SpellEntry const* spellInfo)
{
    ...
    if (!(spellMisc->AttributesEx & SPELL_ATTR_EX_UNAFFECTED_BY_SCHOOL_IMMUNE) &&         // unaffected by school immunity
        !(spellMisc->AttributesEx & SPELL_ATTR_EX_DISPEL_AURAS_ON_IMMUNITY))              // can remove immune (by dispell or immune it)
    {
        SpellImmuneList const& schoolList = m_spellImmune[IMMUNITY_SCHOOL];
        for(SpellImmuneList::const_iterator itr = schoolList.begin(); itr != schoolList.end(); ++itr)
...
uint32 Unit::MeleeDamageBonusDone(Unit *pVictim, uint32 pdamage,WeaponAttackType attType, SpellEntry const *spellProto, DamageEffectType damagetype, uint32 stack)
{
    ...
    if (!pVictim || pdamage == 0 || (spellMisc && spellMisc->AttributesEx6 & SPELL_ATTR_EX6_NO_DMG_MODS))
        return pdamage;
    ...
    uint32 creatureTypeMask = pVictim->GetCreatureTypeMask();
    uint32 schoolMask       = spellProto ? spellMisc->SchoolMask : GetMeleeDamageSchoolMask();
...
int32 Unit::CalculateSpellDamage(Unit const* target, SpellEntry const* spellProto, SpellEffectIndex effect_index, int32 const* effBasePoints)
{
    ...
    if(spellMisc->Attributes & SPELL_ATTR_LEVEL_DAMAGE_CALCULATION && spellLevel &&
...
int32 Unit::CalculateAuraDuration(SpellEntry const* spellProto, uint32 effectMask, int32 duration, Unit const* caster)
{
    ...
    if (caster == this)
    {
        switch(spellProto->GetSpellFamilyName())
        {
            case SPELLFAMILY_DRUID:
                // Thorns
                if (spellMisc->SpellIconID == 53 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000100)))
                {
                    // Glyph of Thorns
            ...
            case SPELLFAMILY_PALADIN:
                // Blessing of Might
                if (spellMisc->SpellIconID == 298 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000000002)))
                {
                ...
                // Blessing of Wisdom
                else if (spellMisc->SpellIconID == 306 && spellProto->IsFitToFamilyMask(UI64LIT(0x0000000000010000)))
                {
...
void Unit::SetPower(Powers power, uint32 val)
{
    ...
    else if(((Creature*)this)->IsPet())
    {
        ...
        // Update the pet's character sheet
...
void Unit::RemoveAurasAtMechanicImmunity(uint32 mechMask, uint32 exceptSpellId, bool non_positive /*= false*/)
{
    ...
    for(Unit::SpellAuraHolderMap::iterator iter = auras.begin(); iter != auras.end();)
    {
        ...
        if (spell->Id == exceptSpellId)
            ...
            ++iter;
        else if (spellMisc->Attributes & SPELL_ATTR_UNAFFECTED_BY_INVULNERABILITY)
            ++iter;
